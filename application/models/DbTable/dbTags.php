...
/**
 * Класс работы с таблицей tags базы данных DB
 *
 * @uses       Model_DBCommon
 * @package    cli-azot-m5
 * @subpackage Model
 */
class Model_DbTable_Tags extends Model_DBCommon {
    ...
    public function __construct() {
        //Получим адаптер
        parent::__construct (array('name'=>"tags",'adapter'=>'DB'));
    }
    ...
    private function _getQuery($aType, array $aParams = null) {
        ...
        switch($aType) {
            ...
            case("get_rows"):
                ...
                break;
            case("get_rows_in"):
                $result = "SELECT * FROM `$table` WHERE `%s` IN(%s)";
                $result = sprintf($result, $aParams[0], $aParams[1]);
                break;
            case("find"):
                $result = "SELECT * FROM `$table` WHERE (%s)";
                $result = sprintf($result, $aParams[0]);
                break;
            default:
                break;
