...
/**
 * Класс работы с таблицей data_day базы данных DB
 *
 * @uses       Model_DBCommon
 * @package    cli-azot-m5
 * @subpackage Model
 */
class Model_DbTable_DayData extends Model_DBCommon {
    ...
    public function __construct() {
        //Получим адаптер
        parent::__construct (array('name'=>"data_day",'adapter'=>'DB'));
    }
    ...
    private function _getQuery($aType, array $aParams = null) {
        ...
        switch($aType) {
            ...
            case("get_data_for_days"):
                ...
                break;
            case("get_rows_for_double"):
                $result = "SELECT *
                           FROM `$table`
                           WHERE `name`='%s' AND `date_hist`='%s'";
                $result = sprintf($result, $aParams[0], $aParams[1]);
                break;
            default:
                break;
