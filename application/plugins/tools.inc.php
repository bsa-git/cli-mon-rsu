...
function check_email($email)
{
	...
	{
		$email = $emailX;
		$emailX = preg_replace('/^((?:[^"\\\\]|\\\\.)*(?:"(?:[^"\\\\]|\\\\.)*"(?:[^"\\\\]|\\\\.)*)*)\\((?:[^()\\\\]|\\\\.)*\\)/s', '$1 ', $email);
	}

	static $email_re = false;
	if(!$email_re)
	{
		$white_space = "(?:(?:\\r\\n)?[ \\t])";
		...
		$phrase = "$word*";
		$route = "(?:\@$domain(?:,\@$white_space*$domain)*:$white_space*)";
		$route_addr = "\\<$white_space*$route?$addr_spec\\>$white_space*";
		$mailbox = "(?:$addr_spec|$phrase$route_addr)";

		$group = "$phrase:$white_space*(?:$mailbox(?:,\\s*$mailbox)*)?;\\s*";
		$address = "(?:$mailbox|$group)";

		$email_re = "$white_space*$address";
	}

	if(preg_match("/^$email_re/s", $email))
		return true;
	else
		return false;
}
...
{
	if(!array_key_exists("fixed_session_id", $_SESSION))
